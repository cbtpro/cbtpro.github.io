...
date: 2026-05-03
tags: ["前端", "后端", "每日精选文章", "命令行", "Flutter", "Android"]
---
...
代码解读
复制代码
iex "& {$(irm get.scoop.sh -Proxy 'http://')} -Proxy 'http://'"
...
附录 app列表 Scoop - Apps bucket列表 Scoop - Buckets
...
在Scoop - Apps （vscode） --- Scoop - Apps (vscode)搜索vscode，选择你要安装的版本来安装。
...
作用：运行后，当你右键点击任何文件夹或文件时，菜单里会出现 “通过 Code 打开” (Open with Code) 的选项。
推荐程度：⭐⭐⭐⭐⭐（必做，极大提升效率）。
...
按下 Win + X，选择 终端（管理员） 或 PowerShell（管理员） 。
依次复制并粘贴你那段提示里的三行命令（以 reg import 开头的）。
每行按回车执行。如果弹出“是否确定要继续”的系统提示，点击是。
⚠️ 特别提醒
