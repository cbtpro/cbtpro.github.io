...
- 仓库允许强推- 仓库是自己学习项目- 会修改原始提交的commit id产生新的commit id
## 修改项目的作者信息
...
使用`rebase -i HEAD^2`、`git commit --amend --no-verify`等命令来进行修改，修改后会使用作者信息进行commit，具体如何使用rebase、merge、commit的操作不在这里展开。

## 修改已push到远端的commit的作者信息

新建一个脚本文件`reset_author.sh`，并增加执行权限
